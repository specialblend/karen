...
export function MyIssueStore(storage: Deno.Kv): Store<Issue> {
  function summarize(issue: Issue) {
    const { key, fields: { summary } } = issue;
    return { key, fields: { summary } };
  }
  return Store<Issue>(["my-issues"], storage, summarize);
}
...
export function fmtIssueWebLink(issue: Issue): URL {
  const selfUrl = new URL(issue.self);
  return new URL(`/browse/${issue.key}`, selfUrl.origin);
}

export function fmtCommentWebLink(issue: Issue, comment: Comment): URL {
  const selfUrl = fmtIssueWebLink(issue);
  selfUrl.searchParams.set("focusedCommentId", comment.id);
  return selfUrl;
}
...
function fmtIssue(data: any): Issue {
  ...
  const fields = {
    project: fmtProject(_fields.project),
    summary,
    description,
    updated,
    created,
...
export function IssueService(storage: Deno.Kv) {
  const FIELDS =
    "id,key,description,summary,updated,created,creator,comment,project";
